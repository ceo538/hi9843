...
class AnalysisPipeline:
    ...
    @staticmethod
    def _priority(event: dict[str, Any], candidates: list[dict], reactions: list[dict]) -> str:
        if event.get("classification") == "DUPLICATE":
            return "P3"
        if any(r.get("classification") in {"STRONG_POSITIVE", "STRONG_NEGATIVE"} for r in reactions):
            return "P0"
        if candidates or event.get("classification") == "UPDATE":
            return "P1"
        return "P2"
    ...
    def analyze_event(
        ...
        *,
        ...
        return {
            "event": event,
            "priority": self._priority(event, candidates, reactions),
